...
public class MoneyBagTools {
    ...
    public String buildBagStr(String content) {
        ...
        Integer hlxInCount = moneybagDetailDao.queryCount("1", "hlx", createDate); //入账
        Integer hlxOutCount = moneybagDetailDao.queryCount("2", "hlx", createDate); //出账
        hlxInCount = hlxInCount==null?0:hlxInCount; hlxOutCount = hlxOutCount==null?0:hlxOutCount;
        ...
        Integer lmsInCount = moneybagDetailDao.queryCount("1", "lms", createDate); //入账
        Integer lmsOutCount = moneybagDetailDao.queryCount("2", "lms", createDate); //出账
        lmsInCount = lmsInCount==null?0:lmsInCount; lmsOutCount = lmsOutCount==null?0:lmsOutCount;
        ...
        Integer qwzwInCount = moneybagDetailDao.queryCount("1", "qwzw", createDate); //入账
        Integer qwzwOutCount = moneybagDetailDao.queryCount("2", "qwzw", createDate); //出账
        qwzwInCount = qwzwInCount==null?0:qwzwInCount; qwzwOutCount = qwzwOutCount==null?0:qwzwOutCount;
       ...
       // sb.append("当月天数：").append(days).append(" 天").append(spe);
        ...
        sb.append("会员充值查询").append(SEP)
            ...
            .append("汉丽轩充值：").append(hlxInCount).append(" 笔").append(SEP)
            .append("汉丽轩消费：").append(hlxOutCount).append(" 笔").append(SEP)
            ...
            .append("兰木肆充值：").append(lmsInCount).append(" 笔").append(SEP)
            .append("兰木肆消费：").append(lmsOutCount).append(" 笔").append(SEP)
            .append(sepLine).append(SEP)
            ...
            .append("签王充值：").append(qwzwInCount).append(" 笔").append(SEP)
            .append("签王消费：").append(qwzwOutCount).append(" 笔").append(SEP)
            .append("请注意核对数据有效性");
        return sb.toString();
    }
